...
namespace F
{
	...
	abstract partial class RegexExpression : ICloneable {
		...
		/// <returns>A new abstract syntax tree representing the expression</returns>
		public static RegexExpression Parse(LexContext pc)
		{
		...
		static IList<RegexCharsetEntry> _ParseRanges(LexContext pc)
		{
			pc.EnsureStarted();
			...
			while (-1 != pc.Current && ']' != pc.Current)
			{
				switch (pc.Current)
				{
					...
					default:
						if (null == next)
						{
							next = new RegexCharsetCharEntry(pc.Current);
						}
						else
						{
							if (readDash)
							{
								result.Add(new RegexCharsetRangeEntry(((RegexCharsetCharEntry)next).Codepoint, pc.Current));
								next = null;
								readDash = false;
							}
							else
		...
		static void _ParseCharClassEscape(LexContext pc, string cls, List<RegexCharsetEntry> result, ref RegexCharsetEntry next, ref bool readDash)
		{
			if (null != next)
			{
				result.Add(next);
		...
		static RegexExpression _ParseModifier(RegexExpression expr,LexContext pc)
		{
			var line = pc.Line;
			var column = pc.Column;
			var position = pc.Position;
	...
	partial class RegexCharsetExpression : RegexExpression, IEquatable<RegexCharsetExpression>
	{
		...
		public override FA ToFA(int accept, bool compact = true)
		{
			...
			for (int ic = Entries.Count, i = 0; i < ic; ++i)
			{
				...
				if (null != crc)
					ranges.Add(new KeyValuePair<int, int>(crc.Codepoint, crc.Codepoint));
				var crr = entry as RegexCharsetRangeEntry;
				if (null != crr)
					ranges.Add(new KeyValuePair<int, int>(crr.FirstCodepoint, crr.LastCodepoint));
